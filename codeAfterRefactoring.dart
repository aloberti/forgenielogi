import 'package:googleapis_auth/auth_io.dart';
import 'package:googleapis/calendar/v3.dart' as calendar;
import 'own_event.dart';

class Borne {
  DateTime? min;
  DateTime? max;
  Borne();
}

bool hasLimitForResearch(String? limit){
  return limit!=null;
}

DateTime fromStringToDate(String mystring){
  final segmentOfString = mystring.split("/");
  return DateTime(int.parse(segmentOfString[2]),int.parse(segmentOfString[1]),int.parse(segmentOfString[0]),23,59,0);
}

Future<calendar.Events> getEventsList(calendar.CalendarApi calendarApi,String calendarId, Borne borne) async {
  return await calendarApi.events.list(
      calendarId,
      orderBy: 'startTime',
      singleEvents: true,
      timeMin: borne.min,
      timeMax: borne.max,
  ); 
}

Borne getEventSearchBorne(String? start, String? end){
  Borne borne = Borne();
  if (hasLimitForResearch(end)){
    borne.max = fromStringToDate(end!);
  }
  if (hasLimitForResearch(start)){
    borne.min = fromStringToDate(start!);
  }
  return borne;
}

List<OwnEvent> analyseAllEvents(calendar.Events events){
  List<OwnEvent> listofevent = [];
  for (var item in events.items ?? []){
    final description = item.description ?? '(Aucune description)';
    final startofevents = item.start?.dateTime ?? item.start?.date;
    final endofevents = item.end?.dateTime ?? item.end?.date;
    final list_of_attendees = item.attendees;
    var attendeespeople = [{"":""}];
    if (list_of_attendees != null && list_of_attendees.isNotEmpty){
      for (var person in list_of_attendees){
        attendeespeople = [];
        attendeespeople.add({"email":person.email ?? "","name":person.displayName ?? ""});
      }
    }
    if (startofevents != null && endofevents != null) {
      listofevent.add(OwnEvent(description, startofevents, endofevents,attendeespeople));
    }
  }
  return listofevent;
}

Future<List<OwnEvent>> geteventscalendar(String? start, String? end) async{
  final accountCredentials  = ServiceAccountCredentials("calendar-reader@projectmcpserveurcalendars9.iam.gserviceaccount.com", ClientId("113768965237292995195"), "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n");
  const scopes = [calendar.CalendarApi.calendarReadonlyScope];
  final client = await clientViaServiceAccount(accountCredentials, scopes);
  final calendarApi = calendar.CalendarApi(client);
  const calendarId = 'fromcpserveurs9@gmail.com';

  var events = calendar.Events();
  Borne borne = getEventSearchBorne(start, end);
  events = getEventsList(calendarApi, calendarId, borne);
  List<OwnEvent> listofevent = analyseAllEvents(events);

  client.close();
  return listofevent;
}